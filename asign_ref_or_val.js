//строка, число и булевый тип всегда передаются по значению, все остальное по ссылке
...
//так как второй массив получен путем ссылки на первый, то через эту ссылку добрались до ориганала.
//Т.е. создвать копию чего либо кроме строк или чисел нужно не через простое присваивание,
//а через, например циклы переберающие оригинал и передающее значеие в копию объекта,
...
//let obj2 = obj1; //здесь мы не скопирвали, а obj2 присвоили ссылку obj1;
//теперь меняя obj2 мы меняем obj1

//obj2.a = 1;
console.log(obj1); //свойство a стало 1, вместо 5;

//объекты удобно перебирать с помошью for in, и перебирая создавать копию 
